...
**Auth: every generation POST below spends the workspace wallet and REQUIRES
`X-Workspace-DB-Token: window.__workspaceDb.token`.** An anonymous request is
rejected with 401 before ElevenLabs is called. The two catalog `GET`s are free
and need no token. Requests are also rate-limited per workspace and script
length is capped, so handle 400/429 responses.

```javascript
const ws = (window as any).__workspaceDb;
const auth = { 'Content-Type': 'application/json', 'X-Workspace-DB-Token': ws.token };
```
...
| `GET /api/music/presets` | List Music Presets |
...
### razorpay-payments
**Category:** General
Accept payments and subscriptions in your Space with Razorpay, using your own Razorpay account keys. Connect mode only: the founder enters Key ID, Key Secret, and Webhook Secret in Wallet, then the standard `/api/payments/checkout` and `/api/payments/subscribe` endpoints route through Razorpay automatically. No trials, intro pricing, promo codes, or saved cards; INR default, amounts in paise. See `integrations/razorpay-payments/docs.md` for setup and limitations.

```typescript
// One-time purchase: Rs 499.00 (amounts in paise)
