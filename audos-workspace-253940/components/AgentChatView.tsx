...
import { LoanleyCalcCard, LoanleyResultCard } from './LoanleyCards';
import { LoanleyCompareCard } from './LenderCompareCard';
...
function createMarkdownComponents(onCalcSubmit: (message: string) => void): Components {
  return {
  ...
  code({ node: _node, children, className, ...props }) {
    // Loanley conversation cards: the agent emits fenced blocks with the
    // languages loanley-calc (inline calculator input card), loanley-compare
    // (neutral lender comparison — requirements form + ranked results) and
    // loanley-result (structured maths/benchmark result card).
    if (typeof className === 'string' && className.includes('language-loanley')) {
      const rawText = Array.isArray(children) ? children.join('') : String(children ?? '');
      if (className.includes('language-loanley-calc')) {
        return <LoanleyCalcCard raw={rawText} onSubmit={onCalcSubmit} />;
      }
      if (className.includes('language-loanley-compare')) {
        return <LoanleyCompareCard raw={rawText} onSubmit={onCalcSubmit} />;
      }
      return <LoanleyResultCard raw={rawText} />;
    }
  ...
  pre({ node: _node, children, ...props }) {
    // Loanley cards render their own container — skip the <pre> chrome.
    const firstChild: any = Array.isArray(children) ? children[0] : children;
    const childClass = firstChild?.props?.className;
    if (typeof childClass === 'string' && childClass.includes('language-loanley')) {
      return <>{children}</>;
    }
    return (
      <pre
...
export default function AgentChatView({ runtime }: AgentChatViewProps) {
  ...
  // Stable markdown components so interactive Loanley cards keep their local
  // state across streaming re-renders (a new components object each render
  // would remount them).
  const sendMessageRef = useRef(sendMessageWithContent);
  sendMessageRef.current = sendMessageWithContent;
  const markdownComponents = useMemo(
    () =>
      createMarkdownComponents((message: string) => {
  ...
  const composerPlaceholder = pendingAttachments.length > 0
    ...
    : isBeaconSpace
      ...
      : 'Paste your loan offer or ask a loan question… e.g. "₹5 lakh at 14% for 4 years — is this good?"';
        ...
        <div className="flex-1 overflow-y-auto flex flex-col justify-center">
          <div className="mx-auto w-full max-w-2xl px-6 sm:px-10 py-10" data-testid="home-view">
            <div className="text-center mb-8">
              ...
              <h1 className="text-2xl font-semibold text-[var(--space-text-primary)]">See clearly. Borrow wisely.</h1>
              {homeWelcomeText && (
      ...
      <div className="flex-1 overflow-auto">
        ...
        {showEmptyStateWelcome ? (
          <div className="mt-8 px-4">
            {isBeaconSpace ? (
              ...
              <div className="text-center text-[var(--space-text-secondary)]">
                ...
                <p className="text-sm">
                  Paste a loan offer or ask any loan question — {agentLabel} does the real maths, shows its sources, and never sells you a lender.
                </p>
                {starterPrompts.length > 0 && (
                  <div className="mt-5 flex flex-wrap justify-center gap-2">
                    {starterPrompts.map((prompt) => (
          ...
          visibleMessages.map((msg, idx) => (
            // Loanley thread layout: user turns right-aligned as compact white
            // bubbles, agent turns left-aligned on the paper off-white.
            <div key={idx} className={msg.role === 'user' ? 'flex justify-end' : undefined}>
              <div
                className={`p-3 rounded-lg overflow-hidden min-w-0 ${
                  msg.role === 'user' ? `${tw.message.user} ml-8 max-w-[85%]` : `${tw.message.assistant} mr-8`
                }`}
              >
        ...
        {streamingContent && (
          <div
            className={`${isBeaconSpace ? 'bg-[var(--space-surface-card)] border border-[var(--space-semantic-success-100)] shadow-sm' : 'bg-[var(--space-surface-muted)]'} mr-8 p-3 rounded-lg overflow-hidden min-w-0`}
          >
